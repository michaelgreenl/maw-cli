...
const createProject = async (): Promise<string> => {
  const root = await mkdtemp(join(tmpdir(), "maw-init-"));
  roots.push(root);
...
describe("maw init", () => {
  ...
  it("creates the phase 2c scaffold in the target project", async () => {
    ...
    expect(await readFile(join(root, ".maw/config.json"), "utf8")).toContain(
      "${OPENAI_API_KEY}",
    );
    expect(await readFile(join(root, ".maw/ov.conf"), "utf8")).toContain(
      "${OPENAI_API_KEY}",
    );
    ...
    await expect(
      readFile(join(root, ".maw/templates"), "utf8"),
    ).rejects.toThrow();
  });
  ...
  it("preserves existing scaffold files and merges gitignore entries once", async () => {
    ...
    expect(await readFile(join(root, ".maw/graph.ts"), "utf8")).toBe(
      "// custom graph\n",
    );
    expect(await readFile(join(root, ".gitignore"), "utf8")).toBe(
      "node_modules/\n.maw/config.json\n.maw/ov.conf\n",
  ...
  it("fails when no installed workflow package exposes the scaffold contract", async () => {
    const root = await mkdtemp(join(tmpdir(), "maw-init-empty-"));
    roots.push(root);
